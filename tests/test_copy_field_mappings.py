...
def _vocab(vocabulary_id):
    ...
    ConceptFactory's SubFactory only ever creates LOINC, and Concept.vocabulary
    is a deferred FK — so passing a bare ``vocabulary_id='SNOMED'`` builds a
    concept pointing at a row that does not exist, and the violation surfaces at
    teardown rather than in the test that caused it.
    """
...
def _seed_code_mapping(**overrides):
    ...
    kwargs = dict(
        ...
        # legitimate and common. An HK-* value would be invalid here — clean()
        # rejects those as source systems — and create() would not catch it.
